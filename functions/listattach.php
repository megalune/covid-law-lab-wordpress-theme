...
$separator = ', ';
	...
	if( count( $attlist ) ) {
		...
		foreach( $attlist as $att ) {
			...
			$attlink = $att->attlink;
			$r .= $atts['before_item'] . '<a href="' . $attlink .'" title="View ' . apply_filters('the_title_attribute',$att->post_title) . '" target="_blank">' . apply_filters('the_title',$att->post_title) . '</a>' . ( ( $showsize ) ? ' <span class="attachment-size">' . la_get_filesize( str_replace( $upload_dir['baseurl'], $upload_dir['basedir'], $attlink ) ) . '</span>' : '' ) . $separator . $atts['after_item'];
		}
		if( $open ) {
			$r .= $atts['closing'] . $atts['after_item'];
		}
