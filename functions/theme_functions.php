...
if (function_exists('add_custom_background')) add_theme_support( 'custom-background');
 ...
 * Function : Fetches category link by category_id.
 * Returns : url for category link
 * Params: category_id @integer
 */
add_action('get-caturl','getcaturl_function');
function getcaturl_function($category_id){
    // Fetches the current page uri .
    $path = $_SERVER['REQUEST_URI'];
    // Fetch the filter value from path 
    $filter = end(explode('+',$path));
    //To check whether filter is added in url or not.
    if($filter != $path){
        // Fetches the filter substring before the seperator '/'
        $filter = substr($filter,0,strpos($filter,'/'));
        // To concat with the filter
        $and='+';
        // Fetches the category link by id.
        $url =  get_category_link($category_id);
        // Trim the seperator so as to add the filter with category.
        $cat_url = rtrim($url,'/');
        // Required link formed 
        $cat_url .=  $and.$filter;
    }else{
